...
const OTAReport = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            label: "Room Assigned",
            options: {
                customBodyRenderLite: (dataIndex) => bookingdata[dataIndex]["room Assigned"],
            },
        },
        {
            name: "roomType",
    ...
    const handelReset = async () => {
        ...
        try {
            const res = await axios.get(
                `/Reports/OTADetails?FromDate=${moment(new Date()).format("YYYY-MM-DD")}&ToDate=${moment(new Date()).format("YYYY-MM-DD")}&FetchType=`,
                {
                    headers: {
                        LoginID,
                        Token,
    ...
    const getBookingData = async () => {
        try {
            const res = await axios.get(`/Reports/OTADetails?FromDate=${moment(fromDate).format('YYYY-MM-DD')}&ToDate=${moment(toDate).format('YYYY-MM-DD')}&FetchType=${dType}`, {
                headers: {
                    LoginID,
                    Token
                }
    ...
    return (
        <>
            <Card>
                <CardHeader>
                    <CardTitle>OTA Report</CardTitle>
                </CardHeader>
