...
const GSTReport = () => {
    ...
    const handelReset = async () => {
        ...
        try {
            const res = await axios.get(
                `/Reports/GSTDetails?FromDate=${moment(subtractMonths(new Date(), 1)).format("YYYY-MM-DD")}&ToDate=${moment(new Date()).format("YYYY-MM-DD")}`,
                {
                    headers: {
                        LoginID,
                        Token,
    ...
    const getcreditData = async () => {
        try {
            const res = await axios.get(`/Reports/GSTDetails?FromDate=${moment(fromDate).format('YYYY-MM-DD')}&ToDate=${moment(toDate).format('YYYY-MM-DD')}`, {
                headers: {
                    LoginID,
                    Token
                }
