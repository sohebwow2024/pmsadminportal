...
const Wizard = forwardRef((props, ref) => {
  ...
  const renderContent = () => {
    return steps.map((step, index) => {
      ...
      // console.log('isActive', isActive, activeIndex, index);
      return (
        <div
          className={classnames("content", {
            [contentClassName]: contentClassName,
            "active dstepper-block": isActive
          })}
          id={step.id}
          key={step.id}
        >
  ...
  //     const isActive = index <= activeIndex;
  //     console.log('isActive', isActive, activeIndex, index);
  //     const classNames = `content ${isActive ? "active dstepper-block" : ""} ${contentClassName}`.trim();
